...
=head1 Description
 ...
 B<cairo_t> is the main object used when drawing with cairo. To draw with cairo, you create a B<cairo_t>, set the target surface, and drawing options for the B<cairo_t>, create shapes with functions like C<cairo_move_to()> and C<cairo_line_to()>, and then draw shapes with C<cairo_stroke()> or C<cairo_fill()>. B<cairo_t> 's can be pushed to a stack via C<cairo_save()>. They may then safely be changed, without losing the current state. Use C<cairo_restore()> to restore to the saved state.
...
Increases the reference count on I<cr> by one. This prevents I<cr> from being destroyed until a matching call to C<cairo_destroy()> is made. Use C<cairo_get_reference_count()> to get the number of references to a B<cairo_t>. Return value: the referenced B<cairo_t>. Since: 1.0
...
Decreases the reference count on I<cr> by one. If the result is zero, then I<cr> and all associated resources are freed. See C<cairo_reference()>. Since: 1.0
...
Return user data previously attached to I<cr> using the specified key. If no user data has been attached with the given key this function returns C<Any>. Return value: the user data previously attached or C<Any>. Since: 1.4

  method cairo_get_user_data ( cairo_user_data_key_t $key )

=item cairo_user_data_key_t $key; a B<cairo_t>

=end pod

sub cairo_get_user_data ( cairo_t $cr, cairo_user_data_key_t $key  )
  is native(&cairo-lib)
  { * }
...
Attach user data to I<cr>. To remove user data from a surface, call this function with the key that was used to set it and C<Any> for I<data>. Return value: C<CAIRO_STATUS_SUCCESS> or C<CAIRO_STATUS_NO_MEMORY> if a slot could not be allocated for the user data. Since: 1.4

  method cairo_set_user_data ( cairo_user_data_key_t $key, OpaquePointer $user_data, cairo_destroy_func_t $destroy --> cairo_status_t )
...
sub cairo_set_user_data ( cairo_t $cr, cairo_user_data_key_t $key, OpaquePointer $user_data, cairo_destroy_func_t $destroy --> cairo_status_t )
  is native(&cairo-lib)
  { * }
}}
...
Returns the current reference count of I<cr>. Return value: the current reference count of I<cr>. If the object is a nil object, 0 will be returned. Since: 1.4
...
sub cairo_get_reference_count ( cairo_t $cr --> uint )
  is native(&cairo-lib)
  { * }
...
Restores I<cr> to the state saved by a preceding call to C<cairo_save()> and removes that state from the stack of saved states. Since: 1.0
...
Temporarily redirects drawing to an intermediate surface known as a group. The redirection lasts until the group is completed by a call to C<cairo_pop_group()> or C<cairo_pop_group_to_source()>. These calls provide the result of any drawing to the group as a pattern, (either as an explicit object, or set as the source pattern). The group will have a content type of I<content>. The ability to control this content type is the only distinction between this function and C<cairo_push_group()> which you should see for a more detailed description of group rendering. Since: 1.2

  method cairo_push_group_with_content ( cairo_content_t $content )

=item cairo_content_t $content; a cairo context

=end pod

sub cairo_push_group_with_content ( cairo_t $cr, cairo_content_t $content  )
  is native(&cairo-lib)
  { * }
...
Terminates the redirection begun by a call to C<cairo_push_group()> or C<cairo_push_group_with_content()> and installs the resulting pattern as the source pattern in the given cairo context. The behavior of this function is equivalent to the sequence of operations: <informalexample><programlisting> cairo_pattern_t *group = cairo_pop_group (cr); cairo_set_source (cr, group); cairo_pattern_destroy (group); </programlisting></informalexample> but is more convenient as their is no need for a variable to store the short-lived pointer to the pattern. The C<cairo_pop_group()> function calls C<cairo_restore()>, (balancing a call to C<cairo_save()> by the push_group function), so that any changes to the graphics state will not be visible outside the group. Since: 1.2
...
Sets the compositing operator to be used for all drawing operations. See B<cairo_operator_t> for details on the semantics of each available compositing operator. The default operator is C<CAIRO_OPERATOR_OVER>. Since: 1.0

  method cairo_set_operator ( cairo_operator_t $op )

=item cairo_operator_t $op; a B<cairo_t>

=end pod

sub cairo_set_operator ( cairo_t $cr, cairo_operator_t $op  )
  is native(&cairo-lib)
  { * }
...
sub cairo_set_source_rgb ( cairo_t $cr, num64 $red, num64 $green, num64 $blue  )
  is native(&cairo-lib)
  { * }
...
Gets the current source pattern for I<cr>. Return value: the current source pattern. This object is owned by cairo. To keep a reference to it, you must call C<cairo_pattern_reference()>. Since: 1.0
...
Set the antialiasing mode of the rasterizer used for drawing shapes. This value is a hint, and a particular backend may or may not support a particular value. At the current time, no backend supports C<CAIRO_ANTIALIAS_SUBPIXEL> when drawing shapes. Note that this option does not affect text rendering, instead see C<cairo_font_options_set_antialias()>. Since: 1.0
  ...
  method cairo_set_antialias ( cairo_antialias_t $antialias )

=item cairo_antialias_t $antialias; a B<cairo_t>

=end pod

sub cairo_set_antialias ( cairo_t $cr, cairo_antialias_t $antialias  )
  is native(&cairo-lib)
  { * }
...
Set the current fill rule within the cairo context. The fill rule is used to determine which regions are inside or outside a complex (potentially self-intersecting) path. The current fill rule affects both C<cairo_fill()> and C<cairo_clip()>. See B<cairo_fill_rule_t> for details on the semantics of each available fill rule. The default fill rule is C<CAIRO_FILL_RULE_WINDING>. Since: 1.0

  method cairo_set_fill_rule ( cairo_fill_rule_t $fill_rule )

=item cairo_fill_rule_t $fill_rule; a B<cairo_t>

=end pod

sub cairo_set_fill_rule ( cairo_t $cr, cairo_fill_rule_t $fill_rule  )
  is native(&cairo-lib)
  { * }
...
Sets the current line cap style within the cairo context. See B<cairo_line_cap_t> for details about how the available line cap styles are drawn. As with the other stroke parameters, the current line cap style is examined by C<cairo_stroke()>, C<cairo_stroke_extents()>, and C<cairo_stroke_to_path()>, but does not have any effect during path construction. The default line cap style is C<CAIRO_LINE_CAP_BUTT>. Since: 1.0

  method cairo_set_line_cap ( cairo_line_cap_t $line_cap )

=item cairo_line_cap_t $line_cap; a cairo context

=end pod

sub cairo_set_line_cap ( cairo_t $cr, cairo_line_cap_t $line_cap  )
  is native(&cairo-lib)
  { * }
...
Sets the current line join style within the cairo context. See B<cairo_line_join_t> for details about how the available line join styles are drawn. As with the other stroke parameters, the current line join style is examined by C<cairo_stroke()>, C<cairo_stroke_extents()>, and C<cairo_stroke_to_path()>, but does not have any effect during path construction. The default line join style is C<CAIRO_LINE_JOIN_MITER>. Since: 1.0
  ...
  method cairo_set_line_join ( cairo_line_join_t $line_join )

=item cairo_line_join_t $line_join; a cairo context

=end pod

sub cairo_set_line_join ( cairo_t $cr, cairo_line_join_t $line_join  )
  is native(&cairo-lib)
  { * }
...
sub cairo_set_dash ( cairo_t $cr, num64 $dashes, int $num_dashes, num64 $offset  )
  is native(&cairo-lib)
  { * }
...
This function returns the length of the dash array in I<cr> (0 if dashing is not currently in effect). See also C<cairo_set_dash()> and C<cairo_get_dash()>. Return value: the length of the dash array, or 0 if no dash array set. Since: 1.4
...
sub cairo_get_dash_count ( cairo_t $cr --> int )
  is native(&cairo-lib)
  { * }
...
Gets the current dash array. If not C<Any>, I<dashes> should be big enough to hold at least the number of values returned by C<cairo_get_dash_count()>. Since: 1.4
...
Modifies the current transformation matrix (CTM) by scaling the X and Y user-space axes by I<sx> and I<sy> respectively. The scaling of the axes takes place after any existing transformation of user space. Since: 1.0
...
Modifies the current transformation matrix (CTM) by rotating the user-space axes by I<angle> radians. The rotation of the axes takes places after any existing transformation of user space. The rotation direction for positive angles is from the positive X axis toward the positive Y axis. Since: 1.0
...
Modifies the current transformation matrix (CTM) by applying I<matrix> as an additional transformation. The new transformation of user space takes place after any existing transformation. Since: 1.0
...
Modifies the current transformation matrix (CTM) by setting it equal to I<matrix>. Since: 1.0
...
Transform a distance vector from user space to device space. This function is similar to C<cairo_user_to_device()> except that the translation components of the CTM will be ignored when transforming (I<dx>,I<dy>). Since: 1.0
...
Transform a distance vector from device space to user space. This function is similar to C<cairo_device_to_user()> except that the translation components of the inverse CTM will be ignored when transforming (I<dx>,I<dy>). Since: 1.0
...
Begin a new sub-path. After this call the current point will be (I<x>, I<y>). Since: 1.0
...
Adds a line to the path from the current point to position (I<x>, I<y>) in user-space coordinates. After this call the current point will be (I<x>, I<y>). If there is no current point before the call to C<cairo_line_to()> this function will behave as cairo_move_to(I<cr>, I<x>, I<y>). Since: 1.0
...
Adds a closed sub-path rectangle of the given size to the current path at position (I<x>, I<y>) in user-space coordinates. This function is logically equivalent to: <informalexample><programlisting> cairo_move_to (cr, x, y); cairo_rel_line_to (cr, width, 0); cairo_rel_line_to (cr, 0, height); cairo_rel_line_to (cr, -width, 0); cairo_close_path (cr); </programlisting></informalexample> Since: 1.0
...
A drawing operator that paints the current source everywhere within the current clip region using a mask of constant alpha value I<alpha>. The effect is similar to C<cairo_paint()>, but the drawing is faded out using the alpha value. Since: 1.0
...
A drawing operator that paints the current source using the alpha channel of I<pattern> as a mask. (Opaque areas of I<pattern> are painted with the source, transparent areas are not painted.) Since: 1.0
...
A drawing operator that paints the current source using the alpha channel of I<surface> as a mask. (Opaque areas of I<surface> are painted with the source, transparent areas are not painted.) Since: 1.0
...
A drawing operator that fills the current path according to the current fill rule, (each sub-path is implicitly closed before being filled). After C<cairo_fill()>, the current path will be cleared from the cairo context. See C<cairo_set_fill_rule()> and C<cairo_fill_preserve()>. Since: 1.0
...
A drawing operator that fills the current path according to the current fill rule, (each sub-path is implicitly closed before being filled). Unlike C<cairo_fill()>, C<cairo_fill_preserve()> preserves the path within the cairo context. See C<cairo_set_fill_rule()> and C<cairo_fill()>. Since: 1.0
...
Emits and clears the current page for backends that support multiple pages. Use C<cairo_copy_page()> if you don't want to clear the page. This is a convenience function that simply calls C<cairo_surface_show_page()> on I<cr>'s target. Since: 1.0
...
Reset the current clip region to its original, unrestricted state. That is, set the clip region to an infinitely large shape containing the target surface. Equivalently, if infinity is too hard to grasp, one can imagine the clip region being reset to the exact bounds of the target surface. Note that code meant to be reusable should not call C<cairo_reset_clip()> as it will cause results unexpected by higher-level code which calls C<cairo_clip()>. Consider using C<cairo_save()> and C<cairo_restore()> around C<cairo_clip()> as a more robust means of temporarily restricting the clip region. Since: 1.0
...
Tests whether the given point is inside the area that would be visible through the current clip, i.e. the area that would be filled by a C<cairo_paint()> operation. See C<cairo_clip()>, and C<cairo_clip_preserve()>. Return value: A non-zero value if the point is inside, or zero if outside. Since: 1.10
...
Create a destination for a hyperlink. Destination tag attributes are detailed at [Destinations][dests]. Since: 1.16 CAIRO_TAG_LINK: Create hyperlink. Link tag attributes are detailed at [Links][links]. Since: 1.16 cairo_tag_begin: Marks the beginning of the I<tag_name> structure. Call C<cairo_tag_end()> with the same I<tag_name> to mark the end of the structure. The attributes string is of the form "key1=value2 key2=value2 ...". Values may be boolean (true/false or 1/0), integer, float, string, or an array. String values are enclosed in single quotes ('). Single quotes and backslashes inside the string should be escaped with a backslash. Boolean values may be set to true by only specifying the key. eg the attribute string "key" is the equivalent to "key=true". Arrays are enclosed in '[]'. eg "rect=[1.2 4.3 2.0 3.0]". If no attributes are required, I<attributes> can be an empty string or NULL. See [Tags and Links Description][cairo-Tags-and-Links.description] for the list of tags and attributes. Invalid nesting of tags or invalid attributes will cause I<cr> to shutdown with a status of C<CAIRO_STATUS_TAG_ERROR>. See C<cairo_tag_end()>. Since: 1.16
...
Marks the end of the I<tag_name> structure. Invalid nesting of tags will cause I<cr> to shutdown with a status of C<CAIRO_STATUS_TAG_ERROR>. See C<cairo_tag_begin()>. Since: 1.16
...
Replaces the current B<cairo_font_face_t> object in the B<cairo_t> with I<font_face>. The replaced font face in the B<cairo_t> will be destroyed if there are no other references to it. Since: 1.0
...
Stores the current font matrix into I<matrix>. See C<cairo_set_font_matrix()>. Since: 1.0
...
Sets a set of custom font rendering options for the B<cairo_t>. Rendering options are derived by merging these options with the options derived from underlying surface; if the value in I<options> has a default value (like C<CAIRO_ANTIALIAS_DEFAULT>), then the value from the surface is used. Since: 1.0
...
Retrieves font rendering options set via B<cairo_set_font_options>. Note that the returned options do not include any options derived from the underlying surface; they are literally the options passed to C<cairo_set_font_options()>. Since: 1.0
...
Replaces the current font face, font matrix, and font options in the B<cairo_t> with those of the B<cairo_scaled_font_t>. Except for some translation, the current CTM of the B<cairo_t> should be the same as that of the B<cairo_scaled_font_t>, which can be accessed using C<cairo_scaled_font_get_ctm()>. Since: 1.2
...
Gets the current scaled font for a B<cairo_t>. Return value: the current scaled font. This object is owned by cairo. To keep a reference to it, you must call C<cairo_scaled_font_reference()>. This function never returns C<Any>. If memory cannot be allocated, a special "nil" B<cairo_scaled_font_t> object will be returned on which C<cairo_scaled_font_status()> returns C<CAIRO_STATUS_NO_MEMORY>. Using this nil object will cause its error state to propagate to other objects it is passed to, (for example, calling C<cairo_set_scaled_font()> with a nil font will trigger an error that will shutdown the B<cairo_t> object). Since: 1.4
...
Gets the extents for a string of text. The extents describe a user-space rectangle that encloses the "inked" portion of the text, (as it would be drawn by C<cairo_show_text()>). Additionally, the x_advance and y_advance values indicate the amount by which the current point would be advanced by C<cairo_show_text()>. Note that whitespace characters do not directly contribute to the size of the rectangle (extents.width and extents.height). They do contribute indirectly by changing the position of non-whitespace characters. In particular, trailing whitespace characters are likely to not affect the size of the rectangle, though they will affect the x_advance and y_advance values. Since: 1.0
...
sub cairo_glyph_extents ( cairo_t $cr, cairo_glyph_t $glyphs, int $num_glyphs, cairo_text_extents_t $extents  )
  is native(&cairo-lib)
  { * }
...
sub cairo_show_glyphs ( cairo_t $cr, cairo_glyph_t $glyphs, int $num_glyphs  )
  is native(&cairo-lib)
  { * }
...
=item cairo_text_cluster_flags_t $cluster_flags; number of clusters in the mapping

=end pod

sub cairo_show_text_glyphs ( cairo_t $cr, Str $utf8, int $utf8_len, cairo_glyph_t $glyphs, int $num_glyphs, cairo_text_cluster_t $clusters, int $num_clusters, cairo_text_cluster_flags_t $cluster_flags  )
  is native(&cairo-lib)
  { * }
...
Adds closed paths for the glyphs to the current path. The generated path if filled, achieves an effect similar to that of C<cairo_show_glyphs()>. Since: 1.0
...
sub cairo_glyph_path ( cairo_t $cr, cairo_glyph_t $glyphs, int $num_glyphs  )
  is native(&cairo-lib)
  { * }
...
Gets the current compositing operator for a cairo context. Return value: the current compositing operator. Since: 1.0

  method cairo_get_operator ( --> cairo_operator_t )


=end pod

sub cairo_get_operator ( cairo_t $cr --> cairo_operator_t )
  is native(&cairo-lib)
  { * }
...
Gets the current tolerance value, as set by C<cairo_set_tolerance()>. Return value: the current tolerance value. Since: 1.0
...
Gets the current shape antialiasing mode, as set by C<cairo_set_antialias()>. Return value: the current shape antialiasing mode. Since: 1.0

  method cairo_get_antialias ( --> cairo_antialias_t )


=end pod

sub cairo_get_antialias ( cairo_t $cr --> cairo_antialias_t )
  is native(&cairo-lib)
  { * }
...
Returns whether a current point is defined on the current path. See C<cairo_get_current_point()> for details on the current point. Return value: whether a current point is defined. Since: 1.6
...
Gets the current fill rule, as set by C<cairo_set_fill_rule()>. Return value: the current fill rule. Since: 1.0

  method cairo_get_fill_rule ( --> cairo_fill_rule_t )


=end pod

sub cairo_get_fill_rule ( cairo_t $cr --> cairo_fill_rule_t )
  is native(&cairo-lib)
  { * }
...
This function returns the current line width value exactly as set by C<cairo_set_line_width()>. Note that the value is unchanged even if the CTM has changed between the calls to C<cairo_set_line_width()> and C<cairo_get_line_width()>. Return value: the current line width. Since: 1.0
...
Gets the current line cap style, as set by C<cairo_set_line_cap()>. Return value: the current line cap style. Since: 1.0

  method cairo_get_line_cap ( --> cairo_line_cap_t )


=end pod

sub cairo_get_line_cap ( cairo_t $cr --> cairo_line_cap_t )
  is native(&cairo-lib)
  { * }
...
Gets the current line join style, as set by C<cairo_set_line_join()>. Return value: the current line join style. Since: 1.0

  method cairo_get_line_join ( --> cairo_line_join_t )


=end pod

sub cairo_get_line_join ( cairo_t $cr --> cairo_line_join_t )
  is native(&cairo-lib)
  { * }
...
Gets the current miter limit, as set by C<cairo_set_miter_limit()>. Return value: the current miter limit. Since: 1.0
...
Stores the current transformation matrix (CTM) into I<matrix>. Since: 1.0
...
Checks whether an error has previously occurred for this context. Returns: the current status of this context, see B<cairo_status_t> Since: 1.0

  method cairo_status ( --> cairo_status_t )


=end pod

sub cairo_status ( cairo_t $cr --> cairo_status_t )
  is native(&cairo-lib)
  { * }
