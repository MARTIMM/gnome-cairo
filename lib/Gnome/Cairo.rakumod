...
method paint-with-alpha ( Num $alpha ) {
  ...
  cairo_paint_with_alpha(
    self._get-native-object-no-reffing, $alpha
  )
}
...
method select-font-face ( Int $slant, Int $weight ) {
  ...
  cairo_select_font_face(
    self._get-native-object-no-reffing, my gint $family, $slant, $weight
  )
}

sub cairo_select_font_face (
  cairo_t $cr, gchar-ptr $family, gint32 $slant, gint32 $weight
) is native(&cairo-lib)
  { * }
...
method set-line-width ( Num $width ) {
  ...
  cairo_set_line_width(
    self._get-native-object-no-reffing, $width
  )
}
...
method set-source ( cairo_pattern_t $source ) {
  ...
  cairo_set_source(
    self._get-native-object-no-reffing, $source
  )
}
...
=item Int $tag_name; a cairo context
=end pod
...
sub cairo_text_extents (
  cairo_t $cr, gchar-ptr $utf8, cairo_text_extents_t $extents
) is native(&cairo-lib)
  { * }
