...
Attach user data to I<cr>. To remove user data from a surface, call this function with the key that was used to set it and C<Any> for I<data>. Return value: C<CAIRO_STATUS_SUCCESS> or C<CAIRO_STATUS_NO_MEMORY> if a slot could not be allocated for the user data. 
  ...
  method cairo_set_user_data ( cairo_user_data_key_t $key, user_data $*, cairo_destroy_func_t $destroy --> cairo_status_t )
...
sub cairo_set_user_data ( cairo_t $cr, cairo_user_data_key_t $key, user_data $*, cairo_destroy_func_t $destroy --> cairo_status_t )
  is native(&cairo-lib)
  { * }
...
Adds a closed sub-path rectangle of the given size to the current path at position (I<x>, I<y>) in user-space coordinates. This function is logically equivalent to: <informalexample><programlisting> cairo_move_to (cr, x, y); cairo_rel_line_to (cr, width, 0); cairo_rel_line_to (cr, 0, height); cairo_rel_line_to (cr, -width, 0); cairo_close_path (cr); </programlisting></informalexample> 
