...
Attach user data to I<pattern>. To remove user data from a surface, call this function with the key that was used to set it and C<Any> for I<data>. Return value: C<CAIRO_STATUS_SUCCESS> or C<CAIRO_STATUS_NO_MEMORY> if a slot could not be allocated for the user data. 
  ...
  method cairo_pattern_set_user_data ( cairo_user_data_key_t $key, user_data $*, cairo_destroy_func_t $destroy --> Int )
...
sub cairo_pattern_set_user_data ( cairo_pattern_t $pattern, cairo_user_data_key_t $key, user_data $*, cairo_destroy_func_t $destroy --> int32 )
  is native(&cairo-lib)
  { * }
...
sub cairo_pattern_get_color_stop_rgba ( cairo_pattern_t $pattern, int $index, num64 $offset, num64 $red, num64 $green, num64 $blue, num64 $alpha --> int32 )
  is native(&cairo-lib)
  { * }
...
sub cairo_pattern_get_color_stop_count ( cairo_pattern_t $pattern, int $count --> int32 )
  is native(&cairo-lib)
  { * }
